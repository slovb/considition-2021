...
class Volume:
    ...
    def __fix_support(self) -> None:
        ...
        if not (self.pos.x - a.dim_x <= a.pos.x <= self.pos.x + self.dim.x) or \
           not (self.pos.y - a.dim_y <= a.pos.y <= self.pos.y + self.dim.y) or \
           not (self.pos.z          <= a.pos.z <= self.pos.z + self.dim.z):
            self.support = None
            return
        # if the area starts outside, crop it
        if a.pos.x < self.pos.x:
            a.dim_x = max(0, a.dim_x - (self.pos.x - a.pos.x))
            a.pos.x = self.pos.x
        if a.pos.y < self.pos.y:
            a.dim_y = max(0, a.dim_y - (self.pos.y - a.pos.y))
            a.pos.y = self.pos.y
        # if the area goes outside, crop it
        a.dim_x = min(a.dim_x, self.dim.x)
        a.dim_y = min(a.dim_y, self.dim.y)
